...
<p class="section-note">Home에는 pinned 공지 1건을 우선 노출하고, 여기에서는 현재 활성 상태인 공지를 모두 확인할 수 있습니다.</p>
    ...
    {% for announcement in sorted_announcements %}
      {% if announcement.expires_at == nil or announcement.expires_at > site.time %}
        ...
        <article class="entry-card entry-card--full">
          ...
          <h2 class="entry-card__title"><a href="{{ announcement.url | prepend: site.baseurl }}">{{ announcement.title }}</a></h2>
...
<p class="section-note"><code>published: false</code> 또는 지난 <code>expires_at</code> 값을 가진 공지는 Home과 이 목록에서 자동으로 숨겨집니다.</p>
